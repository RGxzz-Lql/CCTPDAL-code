        ...
        for line in tqdm(f):
            ...
            if pad_size:
                if len(token) < pad_size:
                    mask = [1] * len(token_ids) + [0] * (pad_size - len(token))#补0补齐
                    ...
                    seq_len = pad_size
            #print(content)
            contents.append((token_ids, int(lable), seq_len, mask,content))
    return contents
...
class DatasetIterator(object):
    ...
    def _to_tensor(self, datas):
        ...
        #content = torch.LongTensor([item[4] for item in datas])
        content = [item[4]for item in datas]
        #n_batches = int(self.n_batches)

        return (x, seq_len, mask), y,content
